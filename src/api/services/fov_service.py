...
def get_satellite_passes_in_fov(
    ...
    prop_strategy = FOVPropagationStrategy()

    for tle in tles:
        sat_start = python_time.time()
        try:
            # Use propagation strategy to get positions
            results = prop_strategy.propagate(
                jd_times,
                tle.tle_line1,
                tle.tle_line2,
                location.lat.value,
            ...
            if results:  # Only process if we found points in FOV
                # Add satellite metadata to results
